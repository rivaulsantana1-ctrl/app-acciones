...
st.markdown("Introduce el ticker de la acción (ej. AAPL, MSFT, TSLA, AMZN) para obtener un análisis completo con gráficos en tiempo real.")

# Input del usuario
ticker = st.text_input("Introduce el Ticker de la acción:", "AAPL").upper()
    ...
    with st.spinner(f"Analizando {ticker} y descargando gráficos..."):
        try:
            ...
            if not info or hist.empty:
                ...
                # Variables básicas (con medidas de seguridad si falta datos)
                ...
                # ---------------------------------------------------------
                st.subheader("📊 Gráficos en Tiempo Real de TradingView")
                
                chart_id = f"tv_chart_{uuid.uuid4().hex[:8]}"
                
                # 1. Widget de Gráfico Avanzado
                st.markdown("### 📈 Gráfico Técnico Avanzado")
                chart_widget = f"""
                ...
                """
                st.components.v1.html(chart_widget, height=520)
                
                # 2. Widget de Análisis Técnico
                st.markdown("### 🔍 Análisis Técnico Automático")
                analysis_widget = f"""
                ...
                """
                st.components.v1.html(analysis_widget, height=320)
